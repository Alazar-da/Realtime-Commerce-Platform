...
const CheckoutPage: React.FC<CheckoutPageProps> = ({ order, onPaymentSuccess }) => {
  ...
  useEffect(() => {
    ...
    const cachedSecret = sessionStorage.getItem(`payment_intent_${order._id}`);
    if (cachedSecret) {
    ...
    const createPaymentIntent = async () => {
      try {
        ...
        // Cache client secret
        sessionStorage.setItem(`payment_intent_${order._id}`, data.clientSecret);
      } catch (err: any) {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (paymentIntent?.status === "succeeded") {
        ...
        // Clear cached secret
        sessionStorage.removeItem(`payment_intent_${order._id}`);
        ...
        toast.info("Additional authentication required. Please follow the prompts.");
      } else {
